...
select evidence_private.sync_registry(jsonb_build_object(
  ...
  'sources', jsonb_build_array(
    jsonb_build_object('source_id', 'fixture_snapshot', 'title', 'Fixture snapshot source', 'publisher', 'Fixture Publisher',
    ...
    jsonb_build_object('source_id', 'fixture_feed', 'title', 'Fixture feed source', 'publisher', 'Fixture Publisher',
...
select ok(evidence_private.acquire_lease('fixture_snapshot', '11111111-1111-1111-1111-111111111111', 60), 'first worker gets the lease');
select ok(not evidence_private.acquire_lease('fixture_snapshot', '22222222-2222-2222-2222-222222222222', 60), 'second worker is refused while the lease is live');
select throws_ok($$select evidence_private.start_run('fixture_snapshot', '22222222-2222-2222-2222-222222222222', 'v1', 'incremental', 'test', 'm1')$$,
  'P0001', null, 'a worker without the lease cannot start a run');
select throws_ok($$select evidence_private.acquire_lease('fixture_snapshot', '11111111-1111-1111-1111-111111111111', 999999)$$,
  'P0001', null, 'lease ttl is bounded');

insert into t select 'run1', evidence_private.start_run('fixture_snapshot', '11111111-1111-1111-1111-111111111111', 'v1', 'incremental', 'test', 'm1');
...
select is((select count(*)::int from evidence_private.ingest_errors where error_class = 'record_rejected'), 6, 'each rejection is recorded');
...
select is((select count(*)::int from evidence_private.source_leases where source_id = 'fixture_snapshot'), 0, 'finishing releases the lease');
...
select evidence_private.acquire_lease('fixture_snapshot', '11111111-1111-1111-1111-111111111111', 60);
insert into t select 'run2', evidence_private.start_run('fixture_snapshot', '11111111-1111-1111-1111-111111111111', 'v1', 'incremental', 'test', 'm1');
...
select is((select count(*)::int from evidence_private.source_record_versions v join evidence_private.source_records r on r.id = v.record_id
            where r.source_id = 'fixture_snapshot'), 12, 'still exactly twelve versions');
select evidence_private.finish_run((select (v ->> 'run_id')::uuid from t where k = 'run2'),
  '11111111-1111-1111-1111-111111111111', 'succeeded', true, 'wm1', null, null);

-- Changed content and one absent record
select evidence_private.acquire_lease('fixture_snapshot', '11111111-1111-1111-1111-111111111111', 60);
insert into t select 'run3', evidence_private.start_run('fixture_snapshot', '11111111-1111-1111-1111-111111111111', 'v1', 'incremental', 'test', 'm1');
...
select evidence_private.acquire_lease('fixture_snapshot', '11111111-1111-1111-1111-111111111111', 60);
insert into t select 'run4', evidence_private.start_run('fixture_snapshot', '11111111-1111-1111-1111-111111111111', 'v1', 'incremental', 'test', 'm1');
...
select is((select count(*)::int from evidence_private.source_records where source_id = 'fixture_snapshot' and tombstoned_at is null), 11,
...
select is((select last_attempt_status || '/' || consecutive_failures from evidence_private.source_freshness where source_id = 'fixture_snapshot'),
  'blocked/1', 'freshness records the failed attempt');
select ok((select last_success_at is not null from evidence_private.source_freshness where source_id = 'fixture_snapshot'),
  'the earlier success time is kept');

-- Safety valve: an almost-empty "complete" snapshot is suspect
select evidence_private.acquire_lease('fixture_snapshot', '11111111-1111-1111-1111-111111111111', 60);
insert into t select 'run5', evidence_private.start_run('fixture_snapshot', '11111111-1111-1111-1111-111111111111', 'v1', 'incremental', 'test', 'm1');
...
-- Reappearance
select evidence_private.acquire_lease('fixture_snapshot', '11111111-1111-1111-1111-111111111111', 60);
insert into t select 'run6', evidence_private.start_run('fixture_snapshot', '11111111-1111-1111-1111-111111111111', 'v1', 'incremental', 'test', 'm1');
...
select evidence_private.acquire_lease('fixture_feed', '11111111-1111-1111-1111-111111111111', 60);
insert into t select 'feed1', evidence_private.start_run('fixture_feed', '11111111-1111-1111-1111-111111111111', 'v1', 'incremental', 'test', 'm1');
...
select evidence_private.acquire_lease('fixture_feed', '11111111-1111-1111-1111-111111111111', 60);
insert into t select 'feed2', evidence_private.start_run('fixture_feed', '11111111-1111-1111-1111-111111111111', 'v1', 'incremental', 'test', 'm1');
