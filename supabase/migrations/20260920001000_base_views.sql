-- Base views: the single definition of every curated, joined view the explorer reads.
-- They live in evidence_views, which is NOT exposed through the REST API and on which no browser
-- role holds any privilege. Two exposed layers are generated from them in
-- 20260920001300_public_projection.sql:
--   evidence_public.<view>     anonymous read, withheld columns removed, release-gated
--   evidence_inspector.<view>  all columns, only for users with a current inspector membership
...
create view evidence_views.import_runs as
...
create view evidence_views.fetch_log as
...
create view evidence_views.record_versions as
...
create view evidence_views.people as
...
create view evidence_views.party_identities as
...
create view evidence_views.identity_decisions as
...
create view evidence_views.service_terms as
...
create view evidence_views.party_affiliations as
...
create view evidence_views.elections as
...
create view evidence_views.electorates as
...
create view evidence_views.candidacies as
...
create view evidence_views.candidacy_status_events as
...
create view evidence_views.documents as
...
create view evidence_views.finance_returns as
...
create view evidence_views.polls as
...
create view evidence_views.stat_series as
...
create view evidence_views.stat_observations as
...
create view evidence_views.stat_route_reconciliation as
...
create view evidence_views.schedules as
...
create view evidence_views.graph_edges as
select * from (
...
begin
  for v_view in
    ...
    where n.nspname = 'evidence_views' and c.relkind = 'v'
  loop
    execute format('alter view evidence_views.%I owner to evidence_inspector_reader', v_view);
    execute format('revoke all on evidence_views.%I from public, anon, authenticated', v_view);
  end loop;
end
$$;
