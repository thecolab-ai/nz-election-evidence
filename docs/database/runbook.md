...
12b. **What an anonymous reader actually receives:** `psql -v ON_ERROR_STOP=1 -f scripts/db/public_value_audit.sql`. Every measurement in it is taken with `set role anon`, so it is the anonymous view and nothing else. Part A is every dataset column by column, Part B is every catalogue product source by source, Part C is the gaps stated as gaps, and **Part D is the allowlisted payload key by key**. Read Part C and Part D before saying a product is published: a projection whose content columns are all null is loaded, not published, and `safe_payload` can read "carries a value" while the reader is receiving two keys of twenty-two.
