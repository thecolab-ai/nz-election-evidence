...
- [x] Database: migration `…001400`; gates and rights rows untouched; `release_basis` reported publicly; per-source, per-field, key-by-key; tier `none` always wins; immutable, revocable, lapsing; administrator only (worker and browser roles denied). pgTAP file `100` (55 assertions)
...
- [x] Verified on a separate disposable stack (own project id and ports; the shared stack was not reset): pgTAP **368/368** on an empty and on a populated database; ingest + tooling **98 pass, 3 skipped (no database selected), 0 fail**; integration **16/16**, skipping forbidden; vitest **41/41**; browser **26/26**; Pages routing **4/4**; generated types match; Python validation, red lines with freeze check, 20 unit tests; copy scan clean (63 files). Rehearsal with real data: 122 / 93 / 10 live rows and the 963-row import, owner file mirrored with the real script, read back as an anonymous client
- [x] Automated adversarial read of the first commit (not a human review): confirmed findings fixed with tests — worker could reset a refused rights row to pending or repoint a source (now administrator only); register rows the parser skips now close the override; more figure names forbidden; sync refuses a missing status, an edited entry and a deleted-but-in-force entry; notice persists while owner-shown fields are in force; UTC dates. Known and documented, not fixed: electorate names are evidenced by the first source to name them
