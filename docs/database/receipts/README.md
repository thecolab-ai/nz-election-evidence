...
Each file is the sanitised receipt of one real run of `ingest/src/cli.ts` on 20 September 2026 (UTC) against the official publisher, written to a **disposable local database**. No hosted project was touched.
...
| `*-nz_parliament_written_questions.run1` | The listing URL redirected to a different host. The fetch guard refused the off-allowlist redirect (`host_denied`). |
| `*-nz_parliament_written_questions.run2` | After the allowlist entry was updated in version control, the page was reachable; status `parser_not_enabled`, nothing imported. |
