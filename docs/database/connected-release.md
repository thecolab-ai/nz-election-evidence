...
>
> **Superseded in one respect, 2026-09-21.** Section 2 below says vote figures, list positions and money are excluded, and section 4 lists a forbidden-name rule that made that a code change rather than a file edit. Both were true of `OWNER-AUTH-2026-09-20-01` and `-02` and are left as written. On 2026-09-21 the owner directed that the public-safe factual values already imported be shown; `OWNER-AUTH-2026-09-21-01` and migration `20260921060100_public_factual_values.sql` do that, through two new scope kinds with closed token lists and their own eligibility rules. Poll figures, sample sizes and anything read from inside a finance return stay out. The current rules — and what is deliberately still withheld — are in [publication-policy.md](publication-policy.md#published-figures-owner-direction-of-2026-09-21). Nothing else in this document changes: the three decisions are still kept apart, no rights row moved, and no gate opened.
