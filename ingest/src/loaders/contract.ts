...
export const LOADER_CONTRACT_VERSION = 1;
...
/** The shared count receipt: input -> ledger -> typed destination, with every check named. */
export interface CountReceipt {
  input: { rows: number | null; records: number | null; versions: number | null };
  written: { seen: number; inserted: number; unchanged: number; rejected: number; conflicts: number; tombstoned: number };
...
export function emptyCounts(): CountReceipt {
  return { input: { rows: null, records: null, versions: null }, written: { seen: 0, inserted: 0, unchanged: 0, rejected: 0, conflicts: 0, tombstoned: 0 }, destination: {}, checks: [] };
}

export function emptyProvenance(adapterName: string, adapterVersion: string): Provenance {
  return { adapter_name: adapterName, adapter_version: adapterVersion, input_digest: null, manifest_hashes: [], run_ids: [], resumed_from_run_ids: [], collected_from: null, collected_to: null };
}
...
export function newReceipt(family: LoaderFamily, unit: LoaderUnit, command: LoaderCommand, adapterName: string, adapterVersion: string): TargetReceipt {
  ...
  return {
    receipt_version: 2, contract_version: LOADER_CONTRACT_VERSION, command, family: family.family, writer: family.writer, unit: unit.unit,
...
/** A finished write command succeeded only if every named check passed and the ledger rejected nothing. */
export function settle(receipt: TargetReceipt, okStatus: LoaderStatus): TargetReceipt {
  if (receipt.counts.written.rejected > 0) {
    receipt.status = "not_reconciled";
    receipt.error_code = "ledger_rejected_rows";
  } else if (receipt.counts.written.conflicts > 0) {
    receipt.status = "not_reconciled";
