...
class CatalogueTests(unittest.TestCase):
 ...
 def test_snapshot_total(self): self.assertEqual(sum(r['record_count'] for r in self.sources),353336)
 def test_all_26_mapped_once(self):
  ids=[r['product_id'] for r in self.sources]; mapped=[p for lane in self.roadmap for p in lane['held_product_ids']]
  self.assertCountEqual(ids,mapped); self.assertEqual(len(mapped),26)
