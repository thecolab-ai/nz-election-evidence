...
test.describe('owner override: a separate, stated decision, never a review and never a publisher approval', () => {
  test('gates closed plus a current owner decision: rows and the named fields of ONE source show, the notice says why, and revoking withholds everything', async ({ page, request }) => {
    ...
    try {
      ...
      await expect(notice).toContainText('Not yet on record')
      await expect(notice).toContainText('Those release gates read closed')
      await expect(notice).toContainText('No publisher has approved or licensed the fields shown')
      ...
      await page.goto('/')
      // Routes exist for all 24 catalogue products; what is HELD is read from the store. The fixture stack holds none of them.
      await expect(page.getByTestId('coverage-missing-note')).toContainText('of the 24 catalogue products')
      ...
      await expect(page.getByTestId('coverage-missing-note')).toContainText(`This store holds ${heldProducts} of the 24 catalogue products; ${missingProducts} are not in it.`)
